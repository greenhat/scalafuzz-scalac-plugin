...
object ScoverageCompiler {
  ...
  val PluginProjectDir = "scalafuzz-scalac-plugin"
  val RuntimeProjectDir = "scalafuzz-scalac-runtime"
  ...
  def settings: Settings = {
    ...
    val path = s"./$PluginProjectDir/target/scala-$ShortScalaVersion/test-generated-classes"
  ...
  private def sbtCompileDir: File = {
    val dir = new File(s"./$PluginProjectDir/target/scala-$ShortScalaVersion/classes")
    if (!dir.exists)
      throw new FileNotFoundException(s"Could not locate SBT compile directory for plugin files [$dir]")
    dir
  }

  private def runtimeClasses: File = new File(s"./$RuntimeProjectDir/jvm/target/scala-$ShortScalaVersion/classes")
...
class ScoverageCompiler(settings: scala.tools.nsc.Settings, reporter: scala.tools.nsc.reporters.Reporter)
  ...
  val instrumentationComponent = new ScalafuzzInstrumentationComponent(this, None, None)
  instrumentationComponent.setOptions(new ScalafuzzOptions())
  ...
  def assertNMeasuredStatements(n: Int): Unit = {
    for ( k <- 1 to n ) {
      assert(testStore.sources.mkString(" ").contains(s"scalafuzz.Invoker.invoked($k,"),
        s"Should be $n invoked statements but missing #$k")
    }
    assert(!testStore.sources.mkString(" ").contains(s"scalafuzz.Invoker.invoked(${n + 1},"),
